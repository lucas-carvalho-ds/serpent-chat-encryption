...
class MainScreen:
    ...
    def build_ui(self, on_room_select_callback, on_send_callback, 
                 on_create_private_callback, on_create_group_callback, on_invite_to_group_callback,
                 on_leave_room_callback, on_view_members_callback, on_logout_callback, on_close_view_callback, 
                 on_rotate_key_callback=None, on_view_keys_callback=None):
        ...
        self.on_rotate_key = on_rotate_key_callback
        ...
        ttk.Button(btn_frame, text="Convidar para Grupo", command=on_invite_to_group_callback).pack(fill="x", pady=2)
    ...
    def rotate_room_key(self):
        """Callback for rotating room key"""
        selection = self.rooms_listbox.curselection()
        if selection:
            index = selection[0]
            item_text = self.rooms_listbox.get(index)
            r_id = int(item_text.split(':')[0])
            if self.on_rotate_key:
                self.on_rotate_key(r_id)
