...
class Database:
    ...
    def _init_db(self):
        ...
        # REMOVIDO: serpent_key (agora fica nos membros)
        cursor.execute('''
        ...
        # ADICIONADO: encrypted_session_key
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS room_members (
                room_id INTEGER NOT NULL,
                user_id INTEGER NOT NULL,
                encrypted_session_key BLOB, -- Chave da sala criptografada com a chave pública do usuário
    ...
    def create_room(self, name, room_type):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('INSERT INTO rooms (name, type) VALUES (?, ?)', (name, room_type))
        conn.commit()
    ...
    def add_room_member(self, room_id, user_id, encrypted_session_key=None):
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('''
                INSERT INTO room_members (room_id, user_id, encrypted_session_key, seen) 
                VALUES (?, ?, ?, 0)
            ''', (room_id, user_id, encrypted_session_key))
            conn.commit()
            return True
        except sqlite3.IntegrityError:
            return False
    ...
    def get_user_room_key(self, room_id, user_id):
        """Retorna a chave criptografada da sala para um usuário específico."""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT encrypted_session_key FROM room_members WHERE room_id = ? AND user_id = ?', (room_id, user_id))
        row = cursor.fetchone()
        conn.close()
        if row:
            return row[0]
    ...
    def update_user_room_key(self, room_id, user_id, new_encrypted_key):
        """Atualiza a chave criptografada de um usuário na sala."""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('UPDATE room_members SET encrypted_session_key = ? WHERE room_id = ? AND user_id = ?', 
                      (new_encrypted_key, room_id, user_id))
        conn.commit()
        conn.close()
